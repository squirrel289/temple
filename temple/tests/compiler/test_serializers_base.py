...
from temple.compiler.serializers.base import (
    Serializer, SerializationError, SerializationContext
)
from temple.compiler.ast_nodes import Text, Expression, Block, SourceRange, Position
...
class MockSerializer(Serializer):
    ...
    def evaluate(self, node, context):
        ...
        if isinstance(node, Expression):
            return str(context.get_variable(node.value))
        if isinstance(node, Block):
            return "".join(str(self.evaluate(c, context)) for c in node.body)
        return ""
...
class TestBasicSerialization:
    ...
    def test_text_serialization(self):
        ...
        source = SourceRange(Position(0, 0), Position(0, 5))
    ...
    def test_expression_serialization(self):
        ...
        source = SourceRange(Position(0, 0), Position(0, 7))
    ...
    def test_block_serialization(self):
        ...
        source = SourceRange(Position(0, 0), Position(0, 1))
        children = [
            Text("Hello ", source),
