...
class ArrayType(BaseType):
    ...
    def validate_value(self, value: Any) -> tuple[bool, Optional[str]]:
        ...
        if self.min_items is not None and len(value) < self.min_items:
            return False, f"Array length {len(value)} is less than minimum {self.min_items}"
    ...
    def to_schema(self) -> Dict[str, Any]:
        ...
        schema = {
            "type": "array",
            "items": self.item_type.to_schema()
        }
...
class ObjectType(BaseType):
    ...
    def is_compatible_with(self, other: BaseType) -> bool:
        ...
        for prop_name in other.required:
            if prop_name not in self.properties:
                return False
            if not self.properties[prop_name].is_compatible_with(other.properties[prop_name]):
                return False
    ...
    def validate_value(self, value: Any) -> tuple[bool, Optional[str]]:
        ...
        if self.min_properties is not None and len(value) < self.min_properties:
            return False, f"Object has {len(value)} properties, minimum is {self.min_properties}"
        
        if self.max_properties is not None and len(value) > self.max_properties:
            return False, f"Object has {len(value)} properties, maximum is {self.max_properties}"
...
class TupleType(BaseType):
    ...
    def validate_value(self, value: Any) -> tuple[bool, Optional[str]]:
        ...
        if len(value) != len(self.item_types):
            return False, f"Expected tuple of length {len(self.item_types)}, got {len(value)}"
...
class UnionType(BaseType):
    ...
    def to_schema(self) -> Dict[str, Any]:
        """Convert to JSON Schema."""
        return {
            "anyOf": [t.to_schema() for t in self.types]
        }
