...
class JSONSerializer(Serializer):
    ...
    def serialize(self, ast: ASTNode, data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            raise SerializationError(f"JSON serialization error: {str(e)}", ast.source_range)
    ...
    def evaluate(self, node: ASTNode, context: SerializationContext) -> Any:
        ...
        if isinstance(node, Text):
            ...
            if not isinstance(iterable, (list, tuple)):
                if self.strict:
                    raise SerializationError(f"For loop requires iterable, got {type(iterable).__name__}", node.source_range)
                return []
            ...
            # Note: Full include handling requires file system access
            # For now, return null
            return None
        
        elif isinstance(node, FunctionDef):
            # Functions not directly serialized; stored in context
            return None
        
        elif isinstance(node, FunctionCall):
            # Function call evaluation (basic support)
            return None
    ...
    def _evaluate_block(self, children: List[ASTNode], context: SerializationContext) -> Any:
        """
