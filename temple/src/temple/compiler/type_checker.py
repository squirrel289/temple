...
class TypeChecker:
    ...
    def _check_if(self, node: If, env: TypeEnvironment) -> BaseType:
        ...
        condition_type = self._check_expression(
            Expression(value=node.condition, source_range=node.source_range),
            env
        )
        ...
        for elif_cond, elif_body in node.elif_parts:
            self._check_expression(
                Expression(value=elif_cond, source_range=node.source_range),
                env
            )
    ...
    def _check_for(self, node: For, env: TypeEnvironment) -> BaseType:
        ...
        iterable_expr = Expression(value=node.iterable, source_range=node.source_range)
        iterable_type = self._check_expression(iterable_expr, env)
        
        # Iterable should be an array
        if not isinstance(iterable_type, ArrayType) and not isinstance(iterable_type, AnyType):
            self.errors.add_error(
                kind=TypeErrorKind.TYPE_MISMATCH,
            ...
            item_type = iterable_type.item_type if isinstance(iterable_type, ArrayType) else AnyType()
    ...
    def _check_function_call(self, node: FunctionCall, env: TypeEnvironment) -> BaseType:
        """Check a function call."""
        # Check arguments
        for arg in node.args:
            self._check_expression(
                Expression(value=arg, source_range=node.source_range),
                env
            )
