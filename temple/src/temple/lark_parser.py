...
class ToTypedAST(Transformer):
    ...
    def if_block(self, items):
        ...
        return If(
            cond,
            body,
            else_if_parts=else_if_parts if else_if_parts else None,
            else_body=else_body,
        )
...
def parse_template(text: str) -> Block:
    ...
    try:
        ...
        if (
            isinstance(res, Tree)
            and len(res.children) == 1
            and isinstance(res.children[0], Block)
        ):
...
def parse_with_diagnostics(text: str) -> Tuple[Block, List[Diagnostic]]:
    ...
    try:
        ...
        source_range = SourceRange(
            Position(line, column), Position(line, column + len(str(e.token)))
        )
